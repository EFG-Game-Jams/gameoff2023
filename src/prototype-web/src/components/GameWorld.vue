...
const playerSelectedNode = ref<IGridNode | null>(null);
...
const onNodeClick = (id: number) => {
  ...
  if (isRunning.value) {
    if (playerSelectedNode.value?.id == id) {
      playerSelectedNode.value = null;
    } else {
      const selectedNode = gridStore.nodes.find((n) => n.id === id);
      if (selectedNode == null) {
        throw new Error(`Selected nonexisting node with ID ${id}`);
      }
      playerSelectedNode.value = selectedNode;
    }
  } else {
    switch (toolMode.value) {
      case "add nodes":
...
const onNodeShiftClick = (id: number) => {
  ...
  if (isRunning.value) {
    ...
    switch (toolMode.value) {
      ...
      case "edge tool":
        const existingSelectIndex = selection.value.findIndex(
          (v) => v.id === id
        );
...
const onSetSelectedNodePriority = (intendedFillRatio: number) => {
  if (intendedFillRatio < 0 || intendedFillRatio > 1) {
    throw Error(`Unsupported fill ratio of ${intendedFillRatio}`);
  }
  if (playerSelectedNode.value == null) {
    console.warn(
      "onSetSelectedNodePriority called without a valid player node selection"
    );
    return;
  }
...
const onToggleSelectedNodeInflow = () => {
  if (playerSelectedNode.value == null) {
    console.warn(
      "onSetSelectedNodePriority called without a valid player node selection"
    );
  ...
  <div class="viewport">
    <div class="grid" @click.exact="onGridClick($event)">
      ...
      <Node
        ...
        :selected="
          isRunning
            ? playerSelectedNode?.id === node.id
            : selection.some((s) => s.id == node.id)
        "
      />
    </div>
    ...
    <div class="controls">
      <template v-if="isRunning">
        <div style="color: darkgray; font-size: 11pt">
          Left click to select nodes <br />
          Press 1, 2 or 3 to set the priority to -1, 0, 1 respectively<br />
        </div>

        <button
          :disabled="playerSelectedNode == null"
          @click="onSetSelectedNodePriority(0)"
        >
          Set priority -1
        </button>
        <button
          :disabled="playerSelectedNode == null"
          @click="onSetSelectedNodePriority(0.5)"
        >
          Set priority 0
        </button>
        <button
          :disabled="playerSelectedNode == null"
          @click="onSetSelectedNodePriority(1)"
        >
          Set priority +1
        </button>
        ...
        <button
          :disabled="playerSelectedNode == null"
          @click="onToggleSelectedNodeInflow()"
        >
          Toggle inflow
        </button>
      </template>
